...
export function DistributorLoginForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_URL}/distributor/auth/login`, {
        method: "POST",
  ...
  return (
    <>
      <h1 className="mb-1 text-center text-2xl font-bold tracking-[-0.02em]">Distributor sign in</h1>
      <p className="text-ink-450 mb-6 text-center text-sm">Onboard and manage your partners.</p>
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="dt-pass" className="text-ink-700 text-[13px] font-semibold">
            Password
          </label>
          <input
            id="dt-pass"
